...
def auto_drop_text(
    ...
    **kwargs,
    ...
    return (
        "<b>#marooow подарки</b>\n"
        "<b>Здарова! 🌟</b>\n\n"
        "Тут ты можешь общаться в комментариях и чате "
        f"и получить <b>{h(prize)}</b> от <b>@bogkm</b>\n\n"
        "Просто общайся в комментариях и получай возможность "
        f"залутать <b>{h(prize)}</b> или <b>NFT подарок</b> 🧸\n\n"
        f"🎲 Шанс выпадения: <b>{chance_percent:g}%</b> за комментарий\n"
        "🎁 Если подарок выпадет — бот ответит на твой коммент\n"
        "👤 Чтобы забрать приз, нужен профиль в боте\n\n"
        "⭐ Купить звезды дешево с любым способом оплаты:\n"
        "<b>@lstarzbot</b>"
    )
