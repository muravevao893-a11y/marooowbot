...
from app.services.referral_service import get_user_chance_bonus, is_admin, track_referral_activity
...
def is_auto_forward_from_channel(message: Message) -> bool:
    ...
    if same_id(getattr(sender_chat, 'id', None), settings.channel_id):
        return True
    return False
...
async def create_auto_drop(message: Message) -> None:
    ...
    async with session_scope() as session:
        ...
        sent = await message.reply(
            auto_drop_text(giveaway.title, giveaway.prize_name, giveaway.ends_at, giveaway.min_participants, settings.chance_drop_percent),
            reply_markup=auto_drop_kb(settings.bot_username, giveaway.id),
        )
        giveaway.announcement_message_id = sent.message_id
        await session.flush()
        log.info('AUTO_DROP_CREATED giveaway_id=%s root=%s thread=%s announcement=%s', giveaway.id, message.message_id, message.message_thread_id, sent.message_id)
...
async def any_message(message: Message) -> None:
    ...
    async with session_scope() as session:
        ...
        if user:
            await track_referral_activity(session, message.bot, settings, user, root_id, message.message_id)
        ...
        if not result.won or result.winner is None:
            log.info('CHANCE_LOST user=%s giveaway=%s final_chance=%s', message.from_user.id, giveaway.id, result.final_chance)
            return

        await message.reply(chance_win_reply_text(giveaway.prize_name), reply_markup=claim_gift_kb(result.winner.id))
        log.info('CHANCE_WIN user=%s giveaway=%s winner=%s final_chance=%s', message.from_user.id, giveaway.id, result.winner.id, result.final_chance)
...
async def claim_chance_gift(callback: CallbackQuery) -> None:
    ...
    async with session_scope() as session:
        ...
        if winner.delivery_status == DeliveryStatus.SENT.value:
            await callback.message.answer(chance_claim_sent_text(winner.giveaway.prize_name))
            if settings.proof_channel_enabled:
                try:
                    await callback.bot.send_message(
                        settings.channel_id,
                        proof_win_text(winner.user, winner.giveaway.prize_name),
                    )
                except Exception:
                    log.exception('Failed to send proof message')
            await callback.answer('Подарок отправлен')
            return
        if winner.delivery_status == DeliveryStatus.MANUAL_REQUIRED.value:
            await callback.message.answer(chance_claim_manual_text(winner.giveaway.prize_name))
...
async def join_manual_giveaway(callback: CallbackQuery) -> None:
    ...
    if is_admin(settings, callback.from_user.id):
        await callback.answer('Админы не участвуют в розыгрышах.', show_alert=True)
        return
    async with session_scope() as session:
        ...
        result = await add_entry(session, callback.bot, settings, giveaway, user, callback.from_user.id, None, EntrySource.BUTTON)
        if not result.ok:
            await callback.answer(result.reason, show_alert=True)
            return
        await callback.bot.edit_message_reply_markup(chat_id=settings.channel_id, message_id=giveaway.channel_message_id, reply_markup=manual_giveaway_kb(giveaway.id, result.count))
        await callback.answer('Ты участвуешь.')
        try:
            await callback.from_user.send_message(joined_text(giveaway.title, result.entry_number or result.count, giveaway.ends_at))
        except Exception:
            pass
